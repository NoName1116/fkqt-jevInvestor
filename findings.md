...
13. Phase 3 不再让 Jev 输出因子占比；Jev 改为输出候选池风险状态和个股方向、延续、量价确认、过热、冲突与数据充分度的有限标签概率。
14. Jev 的状态概率不得表述为未来收益率或下一交易日真实涨跌概率；其统计意义必须在 Phase 6 使用样本外标签单独校准。
15. 候选池状态按正式运行复用一次，个股状态按证券独立评估；持仓状态留给 Phase 4 LLM，不进入 Jev 输入。
16. 动态权重只能由确定性代码将 Jev 候选池概率映射到预定义 Profile，Jev 和 LLM 均不直接生成连续权重。
