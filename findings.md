...
2. Manifest Adapter 验证五类数据的版本、日期、内容哈希、物理 Schema 哈希、行数和路径边界；读取分区目录单文件时必须设置 `partitioning=None`，避免 PyArrow 自动注入 Hive 分区列。
...
12. Alembic 实际数据库已升级并确认处于 `0004_phase2_market_features (head)`。
13. 非阻断警告共 2 条：Starlette TestClient 使用 AnyIO 旧别名；FastAPI 的 `HTTP_422_UNPROCESSABLE_ENTITY` 常量已弃用。两者均来自第三方调用链。
14. Contract 文件相对提交 `057e0c6` 无漂移；`backtest-contract-v1` 标签仍需等待 Contract PR 的非作者 Review 与合并，不能提前发布到未合并提交。
